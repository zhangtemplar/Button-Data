...
class PedailyMaSpider(ButtonSpider):
    name = 'pedaliy_ma'
    allowed_domains = ['zdb.pedaily.cn']
    start_urls = ['https://zdb.pedaily.cn/ma/']
    ...
    def parse(self, response):
        ...
        first_row = True
        result = []
        for row in response.xpath("//ul[@id='inv-list']/li"):
            if first_row:
                first_row = False
                continue
            ...
            stage = row.xpath("dl/dt[@class='money']/span[@class='d']/text()").extract_first()
            unit = row.xpath("dl/dt[@class='money']/span[@class='r']/text()").extract_first()
            amount = row.xpath("dl/dt[@class='money']/span[@class='m']/text()").extract_first()
            if amount is None:
                amount = ''
            else:
                amount = stage + amount
            investor_name = row.xpath("dl/dt[@class='company2']/a/text()").extract_first()
            investor_url = row.xpath("dl/dt[@class='company2']/a/@href").extract_first()
            if investor_name is None:
                investor_name = row.xpath("dl/dt[@class='company2']/text()").extract_first()
            ...
            result.append({
                ...
                'investor': {'name': investor_name, 'url': investor_url},
                'detail': detail})
